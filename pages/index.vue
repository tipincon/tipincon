...
const titles1Text = computed<string[]>(() => t('pages.index.title1').split('[]'))
    ...
    <PageBody class="flex-1 flex">
      <PageSection class="flex-1 flex items-center">
        <div class="flex-1 md:w-5/8 flex flex-col z-10">
          ...
          <h3 class="text-center md:text-left mt-2">
            <span
              v-for="(item, i) in leadings1Text"
              :key="i"
              :style="`--content: '${item.text}'; --start-color: ${
                item.startColor
              }; --end-color: ${item.endColor}; --animation-name: anim-fg-${
              ...
              class="animated-text-bg drop-shadow-xl text-6xl xl:text-6xl 2xl:text-6xl block font-black uppercase"
            >
              <span class="animated-text-fg">{{ item.text }}</span>
            </span>
          </h3>
          ...
          >
            ...
            <Button
              ...
              type="secondary"
              class="font-extrabold"
              href="https://www.youtube.com/@MesonNetwork"
            />
          </div>
        </div>
